...
int main(int argc, char *argv[]) {
    ...
    pthread_t T1_tid;
    create_fp_thread(min_prio + 5, 64*1024, (void*) T1_body, &T1_tid, SCHED_FIFO);
    init_periodic_config(&T2_info);
    pthread_t T2_tid;
    create_fp_thread(min_prio + 6, 64*1024, (void*) T2_body, &T2_tid, SCHED_FIFO);
    init_sporadic_config((thread_config_t*) &T3_info);
    pthread_t T3_tid;
    create_fp_thread(min_prio + 7, 64*1024, (void*) T3_body, &T3_tid, SCHED_FIFO);
