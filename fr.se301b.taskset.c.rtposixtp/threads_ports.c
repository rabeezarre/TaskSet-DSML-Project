...
StatusType SendOutput_runtime(thread_queue_t *global_q, int port_id,
		void *value) {

	StatusType status = E_OK;

	struct port_queue_t *port_q =
			(struct port_queue_t*) global_q->port_queues[port_id];
	...
	if ((port_q->write_idx + 1) % port_q->queue_size == port_q->first_idx) {
		...
		memcpy(
				&port_q->offset[port_q->write_idx
						* (port_q->msg_size / sizeof(char))], value,
				port_q->msg_size);
...
StatusType NextValue_runtime(thread_queue_t *global_q, int port_id, void *dst) {
	...
	memcpy(dst,
			&port_q->offset[(port_q->first_idx)
					* (port_q->msg_size / sizeof(char))], port_q->msg_size);
...
StatusType ReceiveInputs_runtime(thread_queue_t *global_q, int port_id) {
	...
	if (port_q->last_idx < port_q->first_idx)
		discarded_msg_nb = port_q->queue_size - port_q->first_idx
				+ port_q->last_idx;
	else
		discarded_msg_nb = port_q->last_idx - port_q->first_idx;

	if (port_q->write_idx != port_q->last_idx) { // new messages received
...
StatusType GetCount_runtime(thread_queue_t *global_q, int port_id,
		int *count_res) {
	StatusType status = E_OK;
